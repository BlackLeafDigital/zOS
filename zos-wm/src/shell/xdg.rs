...
impl<BackendData: Backend> XdgShellHandler for AnvilState<BackendData> {
    ...
    fn new_toplevel(&mut self, surface: ToplevelSurface) {
        ...
        // Register this toplevel with the foreign-toplevel-list protocol so
        // panels/taskbars can enumerate and track it. Title/app_id are filled
        // in later via `title_changed` / `app_id_changed` once the client sets
        // them; initial values default to empty strings.
        let (title, app_id) = with_states(surface.wl_surface(), |states| {
            let role = states
                .data_map
                .get::<XdgToplevelSurfaceData>()
                .and_then(|d| d.lock().ok().map(|guard| (guard.title.clone(), guard.app_id.clone())));
            match role {
    ...
    fn toplevel_destroyed(&mut self, surface: ToplevelSurface) {
        // Remove the matching foreign-toplevel handle so clients observing the
        // list are informed of the closure.
